...
export class StoreTestimonialDetailGuard implements CanActivate {
  // Variable
  id!: string;
  ...
  private get state$(): Observable<TestimonialData | undefined> {
    return this.store.select(selectTestimonial(this.id));
  }
    ...
    return this.state$.pipe(
      ...
      tap((load) => {
        if (!load) {
          ...
          this.store.dispatch(
            fromTestimonialActions.loadTestimonial({
              id: this.id,
              params,
            })
          );
