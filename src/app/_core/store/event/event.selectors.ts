...
import { State, adapter, ENTITY_FEATURE_KEY } from '@store/event/event.reducers';
...
export const selectEventIds = createSelector(getState, selectIds);

// select the array
export const selectAllEvent = createSelector(getState, selectAll);
...
// select the by title
export const selectEventByTitle = (title: string) =>
  createSelector(getState, (state: State) => {
    return state.entities[title];
  });

// select the by multiple id
export const selectMultipleEvent = (ids: any[]) =>
  createSelector(getState, (state: State) => {
