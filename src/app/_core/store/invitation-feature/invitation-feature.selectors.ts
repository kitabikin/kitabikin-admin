...
import { State, adapter, ENTITY_FEATURE_KEY } from '@store/invitation-feature/invitation-feature.reducers';
...
export const selectInvitationFeatureIds = createSelector(getState, selectIds);

// select the array
export const selectAllInvitationFeature = createSelector(getState, selectAll);

// select the by id
export const selectInvitationFeature = (id: string) =>
  createSelector(getState, (state: State) => {
    return state.entities[id];
  });

// select the by title
export const selectInvitationFeatureByTitle = (title: string) =>
  createSelector(getState, (state: State) => {
    return state.entities[title];
  });

// select the by multiple id
export const selectMultipleInvitationFeature = (ids: any[]) =>
  createSelector(getState, (state: State) => {
