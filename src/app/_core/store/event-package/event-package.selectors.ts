...
import { State, adapter, ENTITY_FEATURE_KEY } from '@store/event-package/event-package.reducers';
...
export const selectEventPackageIds = createSelector(getState, selectIds);

// select the array
export const selectAllEventPackage = createSelector(getState, selectAll);

// select the by id
export const selectEventPackage = (id: string) =>
  createSelector(getState, (state: State) => {
    return state.entities[id];
  });

// select the by title
export const selectEventPackageByTitle = (title: string) =>
  createSelector(getState, (state: State) => {
    return state.entities[title];
  });

// select the by multiple id
export const selectMultipleEventPackage = (ids: any[]) =>
  createSelector(getState, (state: State) => {
