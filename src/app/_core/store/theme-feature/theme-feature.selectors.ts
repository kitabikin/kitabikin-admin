...
import { State, adapter, ENTITY_FEATURE_KEY } from '@store/theme-feature/theme-feature.reducers';
...
export const selectThemeFeatureIds = createSelector(getState, selectIds);

// select the array
export const selectAllThemeFeature = createSelector(getState, selectAll);

// select the by id
export const selectThemeFeature = (id: string) =>
  createSelector(getState, (state: State) => {
    return state.entities[id];
  });

// select the by title
export const selectThemeFeatureByTitle = (title: string) =>
  createSelector(getState, (state: State) => {
    return state.entities[title];
  });

// select the by multiple id
export const selectMultipleThemeFeature = (ids: any[]) =>
  createSelector(getState, (state: State) => {
