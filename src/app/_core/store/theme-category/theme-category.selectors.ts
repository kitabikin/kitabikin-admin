...
import { State, adapter, ENTITY_FEATURE_KEY } from '@store/theme-category/theme-category.reducers';
...
export const selectThemeCategoryIds = createSelector(getState, selectIds);

// select the array
export const selectAllThemeCategory = createSelector(getState, selectAll);

// select the by id
export const selectThemeCategory = (id: string) =>
  createSelector(getState, (state: State) => {
    return state.entities[id];
  });

// select the by title
export const selectThemeCategoryByTitle = (title: string) =>
  createSelector(getState, (state: State) => {
    return state.entities[title];
  });

// select the by multiple id
export const selectMultipleThemeCategory = (ids: any[]) =>
  createSelector(getState, (state: State) => {
