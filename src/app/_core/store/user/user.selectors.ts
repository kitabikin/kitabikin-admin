...
import { State, adapter, ENTITY_FEATURE_KEY } from '@store/user/user.reducers';
...
export const selectUserIds = createSelector(getState, selectIds);

// select the array
export const selectAllUser = createSelector(getState, selectAll);
...
// select the by title
export const selectUserByTitle = (title: string) =>
  createSelector(getState, (state: State) => {
    return state.entities[title];
  });

// select the by multiple id
export const selectMultipleUser = (ids: any[]) =>
  createSelector(getState, (state: State) => {
