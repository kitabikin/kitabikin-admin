...
import { State, adapter, ENTITY_FEATURE_KEY } from '@store/testimonial/testimonial.reducers';
...
export const selectTestimonialIds = createSelector(getState, selectIds);

// select the array
export const selectAllTestimonial = createSelector(getState, selectAll);

// select the by id
export const selectTestimonial = (id: string) =>
  createSelector(getState, (state: State) => {
    return state.entities[id];
  });

// select the by title
export const selectTestimonialByTitle = (title: string) =>
  createSelector(getState, (state: State) => {
    return state.entities[title];
  });

// select the by multiple id
export const selectMultipleTestimonial = (ids: any[]) =>
  createSelector(getState, (state: State) => {
