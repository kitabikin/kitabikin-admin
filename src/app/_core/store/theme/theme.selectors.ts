...
import { State, adapter, ENTITY_FEATURE_KEY } from '@store/theme/theme.reducers';
...
export const selectThemeIds = createSelector(getState, selectIds);

// select the array
export const selectAllTheme = createSelector(getState, selectAll);
...
// select the by title
export const selectThemeByTitle = (title: string) =>
  createSelector(getState, (state: State) => {
    return state.entities[title];
  });

// select the by multiple id
export const selectMultipleTheme = (ids: any[]) =>
  createSelector(getState, (state: State) => {
