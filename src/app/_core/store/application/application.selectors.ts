...
import { State, adapter, ENTITY_FEATURE_KEY } from '@store/application/application.reducers';
...
export const selectApplicationIds = createSelector(getState, selectIds);
...
// select the by title
export const selectApplicationByTitle = (title: string) =>
  createSelector(getState, (state: State) => {
    return state.entities[title];
  });

// select the by multiple id
export const selectMultipleApplication = (ids: any[]) =>
  createSelector(getState, (state: State) => {
