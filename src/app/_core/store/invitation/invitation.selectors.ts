...
import { State, adapter, ENTITY_FEATURE_KEY } from '@store/invitation/invitation.reducers';
...
export const selectInvitationIds = createSelector(getState, selectIds);

// select the array
export const selectAllInvitation = createSelector(getState, selectAll);

// select the by id
export const selectInvitation = (id: string) =>
  createSelector(getState, (state: State) => {
    return state.entities[id];
  });

// select the by title
export const selectInvitationByTitle = (title: string) =>
  createSelector(getState, (state: State) => {
    return state.entities[title];
  });

// select the by multiple id
export const selectMultipleInvitation = (ids: any[]) =>
  createSelector(getState, (state: State) => {
