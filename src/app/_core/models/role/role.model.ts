import { Deserializable, Pagination, ApplicationData } from '@models';
...
export class Role implements Deserializable {
  ...
  deserialize(input: any): this {
    ...
    if (isArray(input.data)) {
      this.data = input.data.map((res: any) => new RoleData().deserialize(res));
    } else {
      this.data = new RoleData().deserialize(input.data);
    }
