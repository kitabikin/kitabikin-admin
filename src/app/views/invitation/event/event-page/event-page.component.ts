...
// MODEL
import { Pagination, EventData } from '@models';
...
import {
  selectAllEvent,
  selectIsLoadingList as selectIsLoadingListEvent,
  selectIsLoadingDelete as selectIsLoadingDeleteEvent,
  selectError as selectErrorEvent,
  selectPagination as selectPaginationEvent,
...
@Component({
  selector: 'kb-event-page',
  templateUrl: './event-page.component.html',
  styleUrls: ['./event-page.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class EventPageComponent implements OnInit, OnChanges, OnDestroy, AfterViewInit {
  ...
  eventIsLoadingList$!: Observable<boolean>;
  eventIsLoadingDelete$!: Observable<boolean>;
  eventIsError$!: Observable<boolean>;
  eventIsError = false;
  eventErrorMessage!: string;
  eventPagination$!: Observable<Pagination>;
  eventPages: any;
  ...
  settingsAll(): void {
    const word = ['event', 'list'];
    this.translateService.get(word).subscribe((trans) => {
      this.label = trans.event;
      this.description = `${trans.list} ${trans.event}`;
  ...
  getAllEvent(): void {
    ...
    this.store
      .pipe(
        select(selectErrorEvent),
        filter((val) => val !== null && val.error)
      )
      .subscribe((result) => {
        if (result.error) {
          this.toastr.error(result.message, 'Event');
        }
      });

    this.eventIsLoadingList$ = this.store.pipe(select(selectIsLoadingListEvent));
    this.eventPagination$ = this.store.pipe(select(selectPaginationEvent));

    this.eventData$ = this.store.pipe(
      select(selectAllEvent),
      map((data: EventData[]) => {
        return data.map((result: any) => new EventData().deserialize(result));
      })
    );
  }
  ...
  onDelete(id: string | undefined, isDelete: boolean = true): void {
    ...
    this.eventIsLoadingDelete$ = this.store.pipe(select(selectIsLoadingDeleteEvent));

    this.store.dispatch(
      fromEventActions.deleteEvent({
        delete: bodyEvent,
      })
    );

    this.actions$
      .pipe(ofType(fromEventActions.deleteEventSuccess), takeUntil(this.unsubscribeEvent$))
      .subscribe((result) => {
        const text = result.data.is_delete ? 'dihapus' : 'dipulihkan';

        Swal.fire({
          icon: 'success',
          text: `Acara ${result.data.name} berhasil ${text}.`,
    ...
    this.actions$
      .pipe(ofType(fromEventActions.deleteEventFailure), takeUntil(this.unsubscribeEvent$))
      .subscribe((result) => {
